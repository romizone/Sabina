...
const BANKING =
  /\b(bank|rekening|tabungan|giro|deposito|saldo|mutasi|transaksi|transfer|bi-?fast|rtgs|skn|qris|atm|kartu|kredit|debit|pinjam|pinjem|minjem|pinjaman|ngutang|utang|cicil|cicilan|plafon|syarat|pengajuan|ajukan|duit|uang|miliar|milyar|juta|kpr|kta|kkb|kur|angsuran|tagihan|limit|pin|otp|cif|ktp|npwp|invest|reksa|obligasi|wealth|bunga|admin|biaya|tarif|rate|jatuh tempo|lokasi|terdekat|cabang|kantor|crm|kurs|valas|usd|lps|ojk|promo|cashback|jam operasional|call center|contact|kontak|hubungi|tiket|pengaduan|komplain|fraud|phishing|blokir|unblokir|va\b|virtual account|paylater|asuransi|bang digital|nasabah|pencairan|statement|koran|nik|ibu kandung|pembayaran|merchant|rrn)\b/i;
...
export function classifyGuard(message: string, hasImage: boolean): GuardDecision {
  ...
  if (isHardOffTopic(text) && !BANKING.test(text)) return "refuse";
